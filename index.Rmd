...
Because it is not an input. The function takes only one input (`x`) for the calculation. `na.rm` specifies whether `NA` values should be removed or not prior to calculation.
...
rescale01_remapped <- function(x) {
  rng <- range(x, na.rm = TRUE, finite = TRUE, Inf == 1, -Inf == 0)
  (x - rng[1]) / (rng[2] - rng[1])
}
```
...
The equations on this website do not render properly.
...
`rnorm()` produces random numbers that are normally distributed, with a mean equal to 0 and a standard deviation equal to 1. In comparison, `MASS::mvrnorm()` produces samples from a multivariate normal distribution. 
