...
`is.finite(x)` indicates which elements of a vector are finite (not `Inf` and not `NA`). `!is.infinite(x)` will return `TRUE` for any elements that are not infinite, including `NA` and `NaN`.
...
It evaluates whether the difference between `x` and `y` is less than the tolerance of comparison (`tol`). This is useful for evaluating the equality of doubles which are approximations because the amount of memory in a computer is fixed.
...
According to the help documentation for `integer` and `double`, an integer vector can take up to about +/-2*10^9. A double can take a range of absolute values from about 2e-308 to 2e+308, however this depends on the CPU.
...
`as.integer` coerces its argument to be of integer type. Similarly, `parse_number` is flexible -- unlike `parse_integer` it ignores floating numbers. You can also round numbers using functions from `round` (e.g.,`floor` will return a vector of integers that have been rounded-down).
